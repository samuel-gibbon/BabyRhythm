...
drumList = os.listdir('D:\\drum_ta\\data\\epoched_v2\\bad_epochs_rejected\\drum')
drumData = []
drumLabel = []

for DrumFilename in drumList: 
    data1 = scipy.io.loadmat('D:\\drum_ta\\data\\epoched_v2\\bad_epochs_rejected\\drum\\'+DrumFilename)
...
taList = os.listdir('D:\\drum_ta\\data\\epoched_v2\\bad_epochs_rejected\\ta')  
taData = []
taLabel = []
 
for TaFilename in taList:
    data2 = scipy.io.loadmat('D:\\drum_ta\\data\\epoched_v2\\bad_epochs_rejected\\ta\\'+TaFilename)
...
with open('classification_results_rejected.csv','w+') as out:
    csv_out=csv.writer(out)
    for row in results:
        csv_out.writerow(row)

model.save("classification_model_rejected.h5")

with open("classification_subjects_rejected.txt", "w+") as subjects:
    for item in drumList:
        subjects.write("{}\n".format(item))

with open('classification_cw_rejected.csv','w+') as out:
    csv_out=csv.writer(out)
    for row in cw:
        csv_out.writerow(row)
